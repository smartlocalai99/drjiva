-- The 20260817120000 doctor_name migration was written against the
-- pre-existing base version of create_hospital_medicine_course, so
-- `create or replace function` silently dropped the unmatched-medicine
-- logging that 20260814090000 had added (the insert into
-- unmatched_medicine_requests on a medicine_not_found skip). Since then,
-- any hospital-billed medicine that doesn't match our catalog has been
-- silently dropped with no record and no daily digest email to the team.
-- This restores that logging while keeping the +91 mobile fix and
-- doctor_name storage already in place.
